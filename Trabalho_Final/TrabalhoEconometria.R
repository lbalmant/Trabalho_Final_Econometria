...
#View(modelo2)  
summary(modelo2)  
#Existe alta correlação entre o crescimento do valor adicionado da industria de trasformação e o crescimento do PIB per capita como esperado
#Não existe correlação entre o crescimento do PIB per capita brasileiro e o ouro branquense, bom dms!!
#intercepto ta horrível
#R² subiu pra caramba
...
summary(modelo3)  
#R²ajustado reduziu pouquissima coisa
#intercepto já está melhor, porém a variação dos residuos foge de uma normal e media = 0
...
summary(modelo4)
#R² quase a mesma coisa
#variância dos residuos se aproximou mais de uma normal e média 0
#tá melhorando
...
summary(modelo5)
#mostrando que o crescimento ou decrescimento da população não foi influente no crescimento do PIB pc
#Ficou horrível!!
...
summary(modelo6)
#melhorou um pouco mas ainda sim o outro estava melhor
...
summary(teste)
#O ajuste do R² não é unicamente devido eu estar trabalhando com PIB pc e valor adicionado. 

#pode ter correlação com a falta de dados --> CAÇAR DADOS DE MAIS ANOS!!//coloquei o maximo q encontrei
...
summary(maisteste)
#retirei a maior parte das variáveis pois o MQO não estava nem rodando.
#nada bom aqui
...
#test rbeush pagan
bptest(modelofinal)
#graficamente e pelo teste bp -> HETEROCEDASTICIDADE
#falei bosta! Na vdd eu acho q conclui homocedasticidade.
...
#presença de heterocedasticidade para o p-valor de 0.06567415
#menor do que 10%
...
#Testanto outras coisas só para desencargo de consciencia
#modteste <- lm(`PIB per capita ouro branquense` ~ `Valor Adicionado - Industria de transformação` + 
#                 I(`Valor Adicionado - Industria de transformação`^2) + 
#                 `Valor Adicionado - administração publica` + 
#                 I(`Valor Adicionado - administração publica`^2)+
#                 `PIB per capita brasileiro`
#                , data = base_final)
#summary(modteste) #fiz um monte de coisa com esse mod teste, ficou tudo ruim.


#residuosteste <- residuals(modteste)
#media <- mean(residuosteste)
#print(media)
